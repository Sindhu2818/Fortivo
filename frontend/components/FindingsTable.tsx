        ...
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr className="text-left text-xs uppercase tracking-wide text-muted-foreground/70">
              <th className={cn(HEADER_CELL, 'w-12 pl-6')}>#</th>
              ...
              <th className={cn(HEADER_CELL, 'w-32 pr-6 text-right')}>Score</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((f) => {
              ...
              return (
                <tr
                  ...
                  className={cn(
                    // No outline-none here: globals.css draws the focus outline on
                    // the row, which a <tr> cannot carry as a box-shadow ring.
                    'cursor-pointer border-b border-border/60 transition-colors last:border-0 hover:bg-muted',
                    // Selected has to read differently from hover, or the open row
                    // is indistinguishable from wherever the cursor happens to be.
                    selected && 'bg-primary/10 hover:bg-primary/[0.15]'
                  )}
                >
                  <td className="py-3 pl-6 pr-3 font-mono tabular-nums text-muted-foreground/70">
                    {f.rank}
                  </td>
