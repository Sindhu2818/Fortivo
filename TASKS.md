...
| 4 | Reduction — dedup + rank down to 30 findings, `occurrences` populated | S | done | `stats.after_dedup < stats.raw_findings` and `len(findings) <= 30` |
| 5 | Scoring — 0–100 risk score, four components, band derivation | S | done | `risk.score` is an int 0–100 |
| 6 | Attack paths — build 2–5 step chains + edges from ranked findings | S | done | `attack_paths` non-empty |
...
| 8 | Pipeline wire-up — scan → normalize → reduce → score → paths → LLM → results | S | done | `/results/<id>` returns a valid document |
...
| 15 | Integration | C+S | in-progress | full chain verified end-to-end 2026-07-26: `./demo-app` → 202 complete in 34.7s, 55→50→30, both scanners in `by_source`, score 92, 5 attack paths. Path-resolution bug fixed in `clone.py`+`validators.py`; `.env` now actually loaded. Remaining: **both LLM keys are wrong** (Gemini quota `limit: 0`, Grok key is a Groq key) so `explanation` is still fallback prose — see STATUS.md blockers |
